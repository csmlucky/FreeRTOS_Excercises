...
int main(void)
{
  ...
  /* USER CODE BEGIN 1 */
	...
	BaseType_t  status;
	char *msg[2] = {"Hello from Task1", "Hello from Task2"};
  ...
  status= xTaskCreate(task1_handler, "Task1", 200, (void *)msg[0], 2, &task1_handle);
  configASSERT(status == pdPASS);

  xTaskCreate(task2_handler, "Task2", 200, (void *)msg[1], 2, &task2_handle);
  configASSERT(status == pdPASS);
